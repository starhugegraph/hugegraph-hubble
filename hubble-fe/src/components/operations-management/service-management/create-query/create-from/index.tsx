...
const CreateFrom = ({
  ...
  useEffect(() => {
    if (isDisable) {
      ...
      api.getQueryDetail(appStore.tenant, detailData.name).then((res: any) => {
        ...
        let newConfigs = keysArr.map((item) => ({
          key: item,
          value: res.data.configs[item]
        }));
  ...
  const onFinish = (values: any) => {
    setLoad(true);
    if (values.urls) {
      values.urls = Array.isArray(values.urls)
        ? values.urls
        : values.urls.split(',').filter((i: string) => i);
    }
    if (values.configs && values.configs.length) {
      let configs = {};
      const newArray = values.configs.map(
    ...
    if (isDisable) {
      api
        .changeQueryDetail(appStore.tenant, detailData.name, values)
        .then(thenCallback);
    } else {
  ...
  return (
    <Skeleton loading={isShowLoading} active>
      <Form {...layout} form={form} name="control-hooks" onFinish={onFinish}>
        ...
        >
          <Select placeholder="请选择运行方式" allowClear disabled={isDisable}>
        ...
        <Form.Item
          ...
          shouldUpdate={(prevValues, currentValues) =>
            prevValues.deployment_type !== currentValues.deployment_type
          }
        >
          {({ getFieldValue }) =>
            getFieldValue('deployment_type') === 'K8S' ? (
              <>
                <Form.Item
                  ...
                  initialValue={1}
                >
                  <Select placeholder="请选择实例个数" allowClear>
                    {OptionArray}
                  </Select>
                </Form.Item>
                          ...
                          >
                            ...
                            >
                              <Select
                                style={{ width: '180px' }}
                                placeholder="请选择配置"
                                onChange={(value) => configChange(value, name)}
                              >
