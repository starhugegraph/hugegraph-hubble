    ...
    return (
        <HeaderRow {...rowProps} style={style}>
            {row.map((cell, i) => {
                ...
                const hasIcon = headerCellProps.hasIcon;
                delete headerCellProps.hasIcon;
                headerCellProps.className = classNames(headerCellProps.className, {
                    [`${prefixCls}-cell-has-icon`]: hasIcon
                });
                return (
                    <HeaderCell {...headerCellProps} key={column.key || column.dataIndex || i} />
                );
            })}
        </HeaderRow>
    );
