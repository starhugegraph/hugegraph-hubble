...
const getResultValue = (originValue, value) => (originValue != null ? originValue : value);
/**
 ...
 */
export const rangeFilter = (value, {max, min, originValue}) => {
    if (isNaN(value)) {
        return NaN;
    }
    let result = value;
 ...
 */
export const fixed = (value, fixed) => {
    value = (typeof value === 'string' || typeof value === 'number') ? `${value}` : '';
 ...
 */
export const numberFormater = (value, config, isRangeFormat) => {
    config = config || {};
...
// 格式化
export const rangeFormater = (value, config) => (`${rangeFilter(value, config)}`);
 ...
 */
export const changeNumber = (initialValue, stepValue, optValue) => {
    const stringStep = `${stepValue}`;
    let initialDigit = 0;
    let stepDigit = 0;
    ...
    const fixed = Math.max(initialDigit, stepDigit);
