...
const computeScaleStyle = memoize(n => ({display: 'inline-block', width: `${n}em`, height: `${n}em`}));

export default (type, svg) => {
    const baseClassName = `one-ui-icon-svg icon-${type}`;
    ...
    return ({className, scale, style, ...props}) => {
        ...
        return <IconComponent {...props} style={iconStyle} className={iconClassName} />;
    };
};
